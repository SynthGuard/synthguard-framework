...
pkgs.mkShell rec {
  ...
  shellHookEntryScript = pkgs.writeShellScript "shellHookEntryScript.sh" ''
    ...
    echo "Creating Portainer namespace if it doesn't exist..."
    kubectl get ns portainer >/dev/null 2>&1 || kubectl create ns portainer
    ...
    # Optional: create Docker registry secret (commented out for security)
